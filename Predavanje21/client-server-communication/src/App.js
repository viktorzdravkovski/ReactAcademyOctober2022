...
function App() {
  ...
  const deletePersonHandler = (id) => {
    setIsLoading(true);
    fetch("https://predavanje-http-communication-default-rtdb.europe-west1.firebasedatabase.app/person/" + id + ".json",
      {
        method: "DELETE",
      }
    ).then((response) => response.json())
  ...
  return (
    <div className="App">
      ...
      <PeopleContainer items={people} deletePersonHandler={deletePersonHandler}/>
    </div>
  );
}
